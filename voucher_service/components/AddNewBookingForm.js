...
const AddNewBookingForm = () => {
  ...
  useEffect(() => {
    ...
    async function getPointsRemaining() {
      ...
      setUserEmail(data.billerInfo.billerEmailAddress);
  ...
  function populateVoucherTypeData() {
    ...
    for (let i in voucherServiceData) {
      list.push(
        <option value={voucherServiceData[i].name}>
          {" "}
          {voucherServiceData[i].name +
            " - " +
            voucherServiceData[i].points +
            " points"}{" "}
        </option>
      );
    }
  ...
  function populateLocationData(serviceName, deliveryMethod) {
    ...
    // pickup from service location
      ...
      for (let i in voucherServiceData) {
        if (voucherServiceData[i].name == serviceName) {
          return (
            <option value={voucherServiceData[i].location}>
              {" "}
              {voucherServiceData[i].location}{" "}
            </option>
          );
        }
      }
  ...
  async function updateUserPoints(serviceName) {
    ...
    if (newPoints < 0) {
      ...
      await dbRef
        .child("users")
        .child(uid)
        .update({ pointsRemaining: newPoints });
  ...
  const handleSubmit = async (event) => {
    ...
    // if points could not be updated successfully, it means that the user doesn't have enough points remaining
      ...
      setNotification(
        "Not enough points remaining to create booking for selected service"
      );
      setTimeout(() => {
        setNotification("");
      }, 2000);
  ...
  return (
    <div>
      ...
      <br />
      <h3>
        <b>Points Remaining: {pointsRemaining}</b>
      </h3>{" "}
      <br />
      {notification}
      <form onSubmit={handleSubmit}>
        <div>
          ...
          <select
            value={voucherType}
            onChange={({ target }) => setVoucherType(target.value)}
          >
