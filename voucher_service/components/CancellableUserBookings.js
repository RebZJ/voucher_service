...
export default function CancellableUserBookings() {
  ...
  function populateBooking() {
    ...
    for (let i in bookings) {
      if (
        (bookings[i].uid == uid && bookings[i].status == "pending") ||
...
function BookingComponent(props, el) {
  ...
  async function upd() {
    ...
    let userEmail = data.billerInfo.billerEmailAddress;
    ...
    await dbRef
      .child("users")
      .child(uid)
      .update({ pointsRemaining: pointsRemaining + pointsNeeded });
  ...
  return (
    <div className=" my-4 shadow-md rounded-lg max-w-sm h-auto p-10 flex flex-col bg-blue-200">
      ...
      <div>
        ...
        <span>
          Status:{" "}
          <select value={statusOf} onChange={updateInfo}>
